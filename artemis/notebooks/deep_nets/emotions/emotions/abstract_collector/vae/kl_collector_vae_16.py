...
image_hists.emotion_histogram = image_hists.emotion_histogram.apply(lambda x: (np.array(x) / float(sum(x))).astype('float16'))
...
create_dir(save_dir)
checkpoint_file = osp.join(save_dir, 'abstract/vae_16/kl_abstract.pt')
...
embedding_,index_=collect_embedding_(model,data_loaders['test'],device)
np.savez(embedding_dir+"/abstract/vae_16/vae_16_test_embedding_kl.npz",embedding=embedding_,index=index_)
print("finished!")
