...
from artemis.neural_models.image_emotion_clf import Emotion_embedding
...
create_dir(save_dir)
checkpoint_file = osp.join(save_dir, 'abstract/softmax_abstract.pt')
...
model = Emotion_embedding(img_encoder, clf_head).to(device)


# collect embedding
model.load_state_dict(torch.load(checkpoint_file))
embedding_,index_=collect_embedding_(model,data_loaders['test'],device)
np.savez(embedding_dir+"/abstract/test_embedding_softmax.npz",embedding=embedding_,index=index_)
print("finished!")
