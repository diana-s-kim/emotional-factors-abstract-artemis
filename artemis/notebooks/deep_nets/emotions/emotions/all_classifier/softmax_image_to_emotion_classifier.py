...
save_dir = './output/'
save_csv_dir ='./csv_save/'
wikiart_img_dir = '/common/home/dsk101/Research/Emotion_Learneing/wikiart'

create_dir(save_dir)
checkpoint_file = osp.join(save_dir, 'all/softmax_all.pt')
...
#save csv
artemis_data.to_csv(save_csv_dir+"all/softmax_all.csv")
...
## prepare data
data_loaders, datasets = image_emotion_label_df_to_pytorch_dataset(artemis_data, args)#label feeding instead of distribution
...
img_encoder = ResnetEncoder('resnet34', adapt_image_size=1).unfreeze(level=6, verbose=True) 
...
#param groups#
all_layer = list(model.img_encoder.resnet.children())#0~7 submodules
group_param=[{'params':all_layer[6].parameters()},{'params':all_layer[7].parameters(),'lr':5e-4},{'params':model.clf_head.parameters(),'lr':1e-3}]
optimizer = torch.optim.Adam(group_param,lr= 1e-4)
...
do_training = False # set to True, if you are not using a pretrained model
max_train_epochs = 5 # test
no_improvement = 0
