...
from artemis.neural_models.image_emotion_clf import VAE_random_embedding
...
artemis_data.to_csv(save_csv_dir+"/all/softmax_all.csv",float_format='%.3f')
...
model = VAE_random_embedding(img_encoder,clf_objective,principle_axes,num_objectives,KL_div).to(device)


# collect embedding
model.load_state_dict(torch.load(checkpoint_file))
embedding_,index_=collect_embedding_(model,data_loaders['train'],device)
np.savez(embedding_dir+"/all_random/vae_32/vae_32_train_embedding_softmax.npz",embedding=embedding_,index=index_)
print("finished!")
